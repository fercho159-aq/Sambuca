        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {branches.map((branch, idx) => (
              <div key={idx} className="bg-card/50 p-6 rounded-2xl border border-border/30 flex flex-col gap-6 group hover:border-primary/50 transition-all duration-300 hover:box-glow-primary">
                <div className="flex justify-between items-start gap-4">
                  <div className="flex items-start gap-4">
                     <PartyPopper className="h-8 w-8 text-primary flex-shrink-0 mt-1" />
                    <div>
                        <h3 className="text-2xl font-bold text-primary">{branch.name}</h3>
                        <div className="flex items-center text-foreground/80 mt-2">
                            <Clock size={16} className="mr-2 text-accent flex-shrink-0" />
                            <p className="text-sm">{branch.hours}</p>
                        </div>
                    </div>
                  </div>
                  <a href={branch.facebookUrl} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-blue-500 transition-colors flex-shrink-0">
                    <Facebook size={24} />
                  </a>
                </div>
                ...
                <div className="w-full h-64 rounded-lg overflow-hidden border-2 border-primary/30 group-hover:border-primary transition-all duration-300 group-hover:box-glow-primary">
                  <iframe
                    src={branch.mapSrc}
                    width="100%"
                    height="100%"
