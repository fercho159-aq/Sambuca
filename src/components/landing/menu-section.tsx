...
import { Bomb, GlassWater, Beer, Pizza, ChefHat, Salad } from "lucide-react";
...
const bebidas = {
  "Neon Drinks": [
    { name: "Pink On", price: "$130", icon: GlassWater, note: "Incluye patito de regalo" },
    { name: "Green On", price: "$130", icon: GlassWater, note: "Incluye patito de regalo" },
    { name: "Blue On", price: "$130", icon: GlassWater, note: "Incluye patito de regalo" },
    { name: "Yellow On", price: "$130", icon: GlassWater, note: "Incluye patito de regalo" },
    { name: "Orange On", price: "$130", icon: GlassWater, note: "Incluye patito de regalo" },
  ],
  "Vaso Sambupato": [
    { name: "Sambujito 1L", price: "$150", icon: DuckIcon, description: "Tradicional, Mango, Fresa, Tamarindo, Maracuyá, Frutos Rojos" },
    { name: "Sambuloca 1L", price: "$150", icon: DuckIcon, description: "Azul, Roja" },
    { name: "Sambuchela 1L", price: "$150", icon: DuckIcon, description: "Clamato, Chela, Natural" },
  ],
  "Shots (10)": [
      { name: "Baby Vodka", price: "$450", icon: Bomb, description: "Cereza, Tamarindo, Mango, Fresa" },
      { name: "Baileys", price: "$300", icon: Bomb },
      { name: "Perlas Negras", price: "$450", icon: Bomb },
      { name: "Shots con Monster o Boost", price: "$300", icon: Bomb },
  ],
  "Sambucolors": [
      { name: "Bacardi & Vodka", price: "$140", icon: BottleIcon, description: "Mango, Fresa, Mora Azul, Manzana Verde. Incluye botella miniatura." },
      { name: "Sambucolors Panditas", price: "$140", icon: BottleIcon, description: "Incluye botella miniatura." },
      { name: "Sambucolors Etiqueta Roja", price: "$140", icon: BottleIcon, description: "Incluye botella miniatura." },
  ],
  "Cervezas y Micheladas": [
    { name: "Monster Chela 1L", price: "$100", icon: Beer },
    { name: "Perla Chela 1L", price: "$100", icon: Beer },
    { name: "Carrón de 20 Cervezas", price: "$748", icon: Beer, description: "Indio, Lager, Tecate, Tecate Ambar, Tecate Light" },
    { name: "Micheladas Especiales 1.25L", price: "$120", icon: Beer, description: "Mango, Cereza, Gomitas, Cacahuates, Cueritos" },
    { name: "Especial de Camarón", price: "$200", icon: Beer, description: "1Lt Chela + Camarón" },
  ]
};
...
const alimentos = {
    ...
    "Para Saborear": [
        { name: "Nuggets de Pollo (6 pz) + Papas", price: "$60" },
        { name: "Papas a la Francesa", price: "$65" },
        { name: "Botanas de Locura", price: "$80", description: "Papas, churritos, gomitas/cacahuates y salsas" },
        { name: "Samburico", price: "$140", description: "6 Alitas, 6 Nuggets, Papas a la Francesa" },
        { name: "Papas de Locura", price: "$80" },
        { name: "Cacahuates Sambuca", price: "$50" },
        { name: "Salchipulpos (6 pz) + Papas", price: "$60" },
    ],
    "Platillos": [
        { name: "Alambres en comal (Pollo o Arrachera)", price: "$220" },
        { name: "Mole (Poblano, Negro, Verde)", price: "$120 - $140", description: "Con pechuga o pierna de pollo" },
    ],
    "Postres": [
        ...
        <TabsContent value="promociones">
            ...
            {promociones.map((promo, index) => (
              <Card key={index} className="bg-card border-2 border-accent/50 hover:border-accent transition-all duration-300 group hover:box-glow-accent flex flex-col">
                 <CardHeader>
                  <CardTitle className="text-3xl font-headline tracking-wider text-accent group-hover:text-glow-accent">{promo.day}</CardTitle>
                </CardHeader>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {paquetes.map((pkg, index) => (
                    <Card key={index} className="bg-card border-2 border-primary/50 hover:border-primary transition-all duration-300 group hover:box-glow-primary">
                        <CardHeader>
                            <CardTitle className="text-4xl font-headline tracking-wider text-primary group-hover:text-glow-primary transition-all duration-300">{pkg.title}</CardTitle>
                        </CardHeader>
                        <CardContent>
        ...
        <TabsContent value="bebidas">
          <div className="space-y-12">
            {Object.entries(bebidas).map(([category, drinks]) => (
              <div key={category}>
                <h3 className="text-4xl md:text-5xl font-headline tracking-widest text-glow-primary mb-6">{category}</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {drinks.map((drink) => (
                    <Card key={drink.name} className="bg-card border border-border/30 flex flex-col">
                        <CardHeader>
                            <div className="flex justify-between items-start">
                                <CardTitle className="text-2xl font-headline tracking-wider text-primary">{drink.name}</CardTitle>
                                <span className="font-mono text-primary text-2xl">{drink.price}</span>
                            </div>
                        </CardHeader>
                      <CardContent className="flex-grow flex flex-col justify-between">
                        <div className="space-y-2">
                             {drink.icon && <drink.icon className="w-8 h-8 text-primary/70 mb-2" />}
        ...
        <TabsContent value="alimentos">
           <div className="space-y-12">
            {Object.entries(alimentos).map(([category, items]) => (
              <div key={category}>
                <h3 className="text-4xl md:text-5xl font-headline tracking-widest text-glow-accent mb-6">{category}</h3>
